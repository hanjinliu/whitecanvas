...
class Namespace(Generic[_C]):
    _attrs: tuple[str, ...] = ()

    def __init__(self, canvas: _C | None = None):
...
class _TextLabelNamespace(Namespace[protocols.CanvasProtocol]):
...
class _AxisNamespace(Namespace[protocols.CanvasProtocol]):
